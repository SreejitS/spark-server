...
// this is where all the user management action happens@
...
RolesController.prototype = {
	...
	addUser: function (userObj) {
		...
		this.usersByUsername[ userObj.username ] = userObj;//user by username@
        
		// if you have access token then give user_id and expiry time@
	...
	saveUser: function (userObj) {
	    var userFolder = path.join(settings.userDataDir,userObj.username);
		if (!fs.existsSync(userFolder)) {
		fs.mkdirSync(userFolder);
		}
		var userFile = path.join(userFolder, userObj.username) + ".json";
	...
	_loadAndCacheUsers: function () {
		...
		for (var i = 0; i < files.length; i++) {
			try {
                var inter = path.join(settings.userDataDir,files[i]);
				var filename = path.join(inter, files[i]+".json");
		...
		PasswordHasher.generateSalt(function (err, userid) {
			...
			PasswordHasher.generateSalt(function (err, salt) {
				...
				PasswordHasher.hash(password, salt, function (err, hash) {
				    ...
				    var userFolder = path.join(settings.userDataDir,username);
					if (!fs.existsSync(userFolder)) {
					fs.mkdirSync(userFolder);
					}
				    if (!fs.existsSync(settings.coreKeysDir)){
					fs.mkdirSync(settings.coreKeysDir);	
					}
				    var coreFolder = path.join(settings.coreKeysDir,userid);
					if (!fs.existsSync(coreFolder)) {
					fs.mkdirSync(coreFolder);
					}
					var userFile = path.join(userFolder, username + ".json");                    
