...
//                    });
    ...
    get_core_key: function(coreid, callback) {
        var dirs = fs.readdirSync(settings.coreKeysDir);
        var counter = 0;
        
        for (var i = 0; i < dirs.length; i++) {
                var userDir = path.join(settings.coreKeysDir,dirs[i]);
                var keyFile = path.join(userDir, coreid +".pub.pem");
    ...
    save_handshake_key: function(userDir, coreid, pem) {

        var keyFile = path.join(userDir, coreid + "_handshake.pub.pem");
        if (!fs.existsSync(keyFile)) {
