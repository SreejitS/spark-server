...
// var username = roles.userObj.username;
...
var DeviceServer = function (options) {
    ...
    this._allCoresByID = {};
    this._attribsByID = {};
    this._allIDs = {};// this is list to be restricted!
...
DeviceServer.prototype = {
    ...
    addCoreKey: function(userid, coreid, public_key) {
        try{
            inbetween1 = path.join(this.options.coreKeysDir,userid);
            var fullPath = path.join(inbetween1, coreid + ".pub.pem");
...
//
		var files1 = fs.readdirSync(this.options.coreKeysDir);
		if (!files1 || (files1.length == 0)) {
			logger.error([ "-------", "No cores exist, you should add cores!", "-------", ].join("\n"));
		}
        
        inbetween2 = path.join(this.options.coreKeysDir,userid);
        var files = fs.readdirSync(inbetween2);
        if(files.length == 0){
            allIDs = null;
        }
        else{
        for (var i = 0; i < files.length; i++) {
            var filename = files[i],// let it be the name for now! lot of action is gonna happen here!
            // inbetween2 = path.join(this.options.coreKeysDir,files[i]);
                fullPath = path.join(inbetween2, filename);
...
//
        ...
        this._allIDs.userid = allIDs;
    },

    saveCoreData: function (userid, coreid, attribs) {
        try {
            //assert basics
            attribs = attribs || {};
            ...
            inbetween = path.join(this.options.coreKeysDir,userid);
            var fullPath = path.join(inbetween, coreid + ".json");
            // fullPath = path.join(this.options.coreKeysDir, coreid + ".json");
     ...
     * @returns {null}
     */
    getAllCoreIDs: function (userid) {
        return this._allIDs.userid;
    },
